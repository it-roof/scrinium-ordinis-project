...
import { PromptContentDiff } from "@/components/v1/prompts/prompt-content-diff";
...
export function V1PromptForm({
  ...
  const isEditPage = !isPanel && mode === "edit";
  ...
  if (isEditPage) {
    return (
      <div className="mx-auto flex min-h-0 w-full max-w-7xl flex-1 flex-col gap-6 overflow-y-auto p-4 md:p-6">
        ...
        <div className="space-y-1">
          ...
          <p className="text-sm text-muted-foreground">
            Links bearbeiten — rechts erscheinen die Änderungen am Prompt-Text
            automatisch.
          </p>
        </div>

        <div className="grid min-h-0 flex-1 gap-6 lg:grid-cols-2">
          <Card className="gap-0 self-start border-border/80 py-0 shadow-none">
          ...
          <div className="min-h-0 lg:sticky lg:top-4 lg:max-h-[calc(100dvh-6rem)] lg:overflow-hidden">
            <PromptContentDiff
              before={originalContent}
              after={content}
              layout="stack"
              className="h-full min-h-[24rem] rounded-xl border border-border/70 bg-muted/15 p-4"
            />
          </div>
        </div>
      </div>
